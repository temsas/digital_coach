...
def generate_contextual_questions(topic, relevant_sections, theory):
    """Генерация вопросов для теста - СНАЧАЛА УЧЕБНИКИ, ПОТОМ ИНТЕРНЕТ"""
    try:
        # Определяем, можно ли использовать внешние знания
        use_external = should_use_external_knowledge(topic, relevant_sections)
        
        if use_external:
            source_instruction = "Используй информацию из учебников и, если нужно, дополнительные знания."
        else:
            source_instruction = "Используй ТОЛЬКО информацию из учебников. НЕ используй дополнительные знания."
...
5. Объяснение должно быть основано на учебниках
6. ДОЛЖНО БЫТЬ РОВНО 5 ВОПРОСОВ
...
{{
    "questions": [
        {{
            ...
            "correct_answer": 0,
            "explanation": "Объяснение на основе учебников..."
...
"""
        ...
        # Если получили меньше 5 вопросов и разрешены внешние знания, пробуем еще раз
        if len(questions) < 5 and use_external:
            logger.warning(f"⚠️ Получено только {len(questions)} вопросов, повторяем генерацию с внешними знаниями")
