...
def check_full_test():
    """Проверка всего теста из 10 вопросов"""
    try:
        data = request.get_json()
...
def create_quality_question(question_id, topic, relevant_sections):
    """Создание качественного вопроса с разными вариантами ответов"""
    if not relevant_sections:
    ...
    if meaningful_sentences:
        # Используем разные предложения для вопроса и правильного ответа
        base_sentence = meaningful_sentences[0]
        correct_answer = meaningful_sentences[1 % len(meaningful_sentences)] if len(meaningful_sentences) > 1 else base_sentence
        
        # Создаем разные неправильные варианты
        wrong_answers = [
            "Это утверждение не соответствует информации в руководстве",
            "В руководстве представлена противоположная информация",
            "Данный вариант не подтверждается содержанием руководства",
            "Это распространенное заблуждение, не связанное с руководством"
        ]
        
        # Перемешиваем варианты
        options = [correct_answer[:80] + "..."] + wrong_answers[:3]
    ...
    return {
        ...
        'options': options,
        'correct_answer': 0,
        'explanation': f'Правильный ответ основан на информации из раздела "{section["title"]}" (страница {section["page"]}).'
    }
