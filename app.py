...
def generate_contextual_questions(topic, relevant_sections, theory):
    """Генерация контекстных вопросов по теме - С ИСПОЛЬЗОВАНИЕМ КНИГИ И ВНЕШНИХ ЗНАНИЙ"""
    try:
        # Определяем, нужно ли использовать внешние знания
        use_external = should_use_external_knowledge_for_test(topic, relevant_sections)
...
1. УЧЕБНИК ПО ЦИФРОВОЙ ГРАМОТНОСТИ:
{format_sections_for_analysis(relevant_sections) if relevant_sections else "В учебнике нет конкретной информации по этой теме."}

2. ТЕОРЕТИЧЕСКАЯ СПРАВКА (на основе учебника и дополнительных знаний):
{theory}

3. ДОПОЛНИТЕЛЬНЫЕ ЗНАНИЯ:
Используй свои современные знания о цифровых технологиях, кибербезопасности и актуальных тенденциях.

ВАЖНЫЕ ПРАВИЛА ГЕНЕРАЦИИ ВОПРОСОВ:
...
6. Вопросы должны проверять как базовые знания, так и понимание современных аспектов

ТИПЫ ВОПРОСОВ ДЛЯ РАЗНООБРАЗИЯ:
- 2 вопроса на базовые понятия (из учебника)
- 2 вопроса на практическое применение (комбинация учебника и современных знаний)  
- 1 вопрос на современные тенденции (преимущественно внешние знания)

ПРИМЕРЫ ВОПРОСОВ ДЛЯ РАЗНЫХ ТЕМ:

Тема: "Безопасность в интернете"
- Базовый: "Что такое фишинг согласно руководству?"
- Практический: "Как создать надежный пароль для онлайн-банкинга?"
- Современный: "Какие меры защиты важны при использовании публичного Wi-Fi?"

Тема: "Электронная почта"  
- Базовый: "Как прикрепить файл к письму в почтовом сервисе?"
- Практический: "Как отличить фишинговое письмо от настоящего?"
- Современный: "Какие альтернативы электронной почте популярны сегодня?"
...
{{
    "questions": [
        {{
            ...
            "correct_answer": 0,
            "explanation": "Детальное объяснение с ссылками на учебник и современные практики..."
        }},
        {{
...
"""
        ...
        # Улучшаем объяснения в вопросах
        questions = enhance_questions_with_knowledge(questions, topic, relevant_sections)
        ...
        logger.info(f"✅ Сгенерировано {len(questions)} вопросов (использованы: учебник + внешние знания)")
...
ВЕРНИ ТОЛЬКО УЛУЧШЕННОЕ ОБЪЯСНЕНИЕ БЕЗ ДОПОЛНИТЕЛЬНЫХ КОММЕНТАРИЕВ.
"""
    ...
    try:
        ...
        return explanation  # Возвращаем оригинальное объяснение при ошибке
...
def generate_contextual_theory_for_test(topic, relevant_sections):
    """Генерация теоретической справки для тестов - с акцентом на тестируемые аспекты"""
    prompt = f"""
СОЗДАЙ ТЕОРЕТИЧЕСКУЮ СПРАВКУ ДЛЯ ТЕСТА ПО ТЕМЕ: "{topic}" 
С АКЦЕНТОМ НА КЛЮЧЕВЫЕ АСПЕКТЫ, КОТОРЫЕ БУДУТ ПРОВЕРЯТЬСЯ В ВОПРОСАХ

ИНФОРМАЦИЯ ИЗ УЧЕБНИКА:
{format_sections_for_analysis(relevant_sections) if relevant_sections else "В учебнике нет конкретной информации по этой теме."}

ДОПОЛНИТЕЛЬНЫЕ ИСТОЧНИКИ:
Используй свои современные знания, чтобы сделать справку актуальной и полезной для тестирования.

СТРУКТУРА ДЛЯ ТЕСТИРОВАНИЯ:
🎯 **Основные понятия** (что будет проверяться в базовых вопросах)
- Ключевые термины и определения
- Основные принципы работы

🛠️ **Практическое применение** (что будет проверяться в практических вопросах)  
- Как использовать на практике
- Типичные сценарии применения
- Пошаговые инструкции

🚀 **Современные аспекты** (что будет проверяться в вопросах на современные тенденции)
- Новые технологии и сервисы
- Актуальные угрозы и защиты
- Последние обновления и тренды

⚠️ **Важные предупреждения** (что может быть в вопросах на безопасность)
- Распространенные ошибки
- Меры предосторожности
- Признаки проблем

ТРЕБОВАНИЯ:
1. Сделай акцент на аспектах, которые важны для тестирования знаний
2. Включи информацию, которая поможет ответить на тестовые вопросы
3. Сочетай базовые знания из учебника с современными практиками
4. Выдели ключевые моменты, которые должны запомниться
...
"""
    ...
    try:
        ...
        return generate_contextual_theory(topic, relevant_sections)  # Используем обычную версию
...
def generate_contextual_theory(topic, relevant_sections):
    """Генерация теоретической справки с использованием книги и внешних знаний"""
    prompt = f"""
СОЗДАЙ КРАСИВОЕ, СТРУКТУРИРОВАННОЕ ОБЪЯСНЕНИЕ ПО ТЕМЕ: "{topic}" С ЧЕТКИМИ АБЗАЦАМИ

ИНФОРМАЦИЯ ИЗ УЧЕБНИКА ПО ЦИФРОВОЙ ГРАМОТНОСТИ:
{format_sections_for_analysis(relevant_sections) if relevant_sections else "В учебнике нет конкретной информации по этой теме."}

ДОПОЛНИТЕЛЬНЫЕ ИСТОЧНИКИ:
Ты можешь использовать свои современные знания о цифровой грамотности и кибербезопасности, чтобы дополнить информацию из учебника.
...
- ЕСЛИ в учебнике есть информация по теме - используй её как основу
- ЕСЛИ информации в учебнике недостаточно - дополни своими знаниями
- ЕСЛИ в учебнике нет информации - полностью полагайся на свои знания
- ВСЕГДА указывай актуальную и проверенную информацию
...
"""
    ...
    try:
        ...
        # Создаем объяснение с использованием внешних знаний
...
def should_use_external_knowledge(topic, relevant_sections):
    """Определяет, нужно ли использовать внешние знания"""
    if not relevant_sections:
        return True
    
    # Проверяем качество найденных разделов
    total_content_length = sum(len(section['content']) for section in relevant_sections)
    if total_content_length < 200:  # Слишком мало информации в учебнике
        return True
    ...
    matches = len(topic_words.intersection(content_keywords))
    if matches < len(topic_words) * 0.5:  # Меньше 50% совпадений
        return True
...
def generate_contextual_test(topic, relevant_sections):
    """Генерация теста с контекстным анализом - С ИСПОЛЬЗОВАНИЕМ КНИГИ И ВНЕШНИХ ЗНАНИЙ"""
    logger.info(f"🔄 Генерация теста по теме '{topic}' (учебник + внешние знания)...")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if has_question_variety(questions):
                logger.info(f"✅ Тест создан: {len(questions)} разнообразных вопросов")
                return {
                    'topic': topic,
                    'theory': theory,
                    'questions': questions,
                ...
                logger.warning(f"⚠️ Попытка {attempt + 1}: вопросы недостаточно разнообразны")
                
        except Exception as e:
            logger.error(f"❌ Попытка {attempt + 1} не удалась: {e}")
    
    # Если все попытки провалились, возвращаем базовый тест
    return create_quality_full_test(topic, relevant_sections)

def has_question_variety(questions):
    """Проверяет, достаточно ли разнообразны вопросы"""
    if len(questions) < 5:
        return False
    
    # Проверяем, что вопросы не слишком похожи друг на друга
    ...
    # Если в 5 вопросах меньше 15 уникальных слов значительной длины - недостаточно разнообразия
...
def learn_topic():
    """Генерация только теоретического объяснения по теме - С ИСПОЛЬЗОВАНИЕМ КНИГИ И ВНЕШНИХ ЗНАНИЙ"""
    if not GIGACHAT_AVAILABLE:
        return jsonify({
            'status': 'error', 
            'error': 'GigaChat недоступен'
    ...
    try:
        ...
        if use_external:
            logger.info("🔍 Использую внешние знания для дополнения информации")
        
        # Генерируем качественное объяснение с использованием книги и внешних знаний
        logger.info("🔄 Генерация объяснения с использованием книги и внешних знаний...")
        explanation = generate_contextual_theory(topic, relevant_sections)
        
        # Дополнительно обогащаем внешними знаниями при необходимости
        if use_external and len(explanation) < 800:  # Если объяснение слишком короткое
            logger.info("🔄 Дополняю объяснение внешними знаниями...")
            explanation = enhance_with_external_knowledge(explanation, topic)
        ...
        logger.info(f"✅ Объяснение создано: {len(explanation)} символов, {explanation.count('\\n\\n') + 1} абзацев")
