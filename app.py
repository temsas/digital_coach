...
def generate_contextual_questions(topic, relevant_sections, theory):
    """Генерация вопросов для теста - ГАРАНТИРОВАННО 5 ВОПРОСОВ"""
    max_attempts = 10
    ...
    for attempt in range(max_attempts):
        try:
            ...
            use_external = True  # Всегда true для тестов
            
            # Усиленный промпт с явным требованием 5 вопросов
            prompt = f"""
ТЫ — ЭКСПЕРТ ПО СОЗДАНИЮ ТЕСТОВ. СОЗДАЙ ТЕСТ ИЗ РОВНО 5 ВОПРОСОВ ПО ТЕМЕ: "{topic}".

ВАЖНО: ТЫ ДОЛЖЕН ВЕРНУТЬ РОВНО 5 ВОПРОСОВ. НЕ МЕНЬШЕ И НЕ БОЛЬШЕ!
...
6. Не повторяй вопросы
7. Не используй в вопросах и вариантах ответов символы #, *, `
...
"""
            ...
            # Если получили больше вопросов, обрезаем до нужного количества
            elif len(questions) > target_question_count:
                logger.info(f"⚠️ Получено больше вопросов ({len(questions)}), обрезаем до {target_question_count}")
    ...
    # Если все попытки провалились, возвращаем то, что есть, или пустой список
    logger.error(f"❌ Не удалось сгенерировать {target_question_count} вопросов после {max_attempts} попыток")
...
def generate_contextual_theory_for_test(topic, relevant_sections):
    ...
    try:
        prompt = f"""
СОЗДАЙ КРАТКУЮ ТЕОРЕТИЧЕСКУЮ СПРАВКУ ДЛЯ ТЕСТА ПО ТЕМЕ: "{topic}" 
(МАКСИМУМ 1500 СИМВОЛОВ)

ИСПОЛЬЗУЙ ИНФОРМАЦИЮ ИЗ УЧЕБНИКОВ И СВОИ ЗНАНИЯ:

ИНФОРМАЦИЯ ИЗ УЧЕБНИКОВ:
{format_sections_for_analysis(relevant_sections) if relevant_sections else "Используй свои знания."}

СТРУКТУРА:
🎯 **Основные понятия** (самое важное)
🛠️ **Практическое применение** (ключевые моменты)  
⚠️ **Безопасность** (главные правила)

ТРЕБОВАНИЯ:
1. Будь кратким и информативным
2. Сделай акцент на аспектах, важных для тестирования
3. НЕ используй символ # для заголовков
4. МАКСИМУМ 1500 символов
...
"""
        ...
        logger.info(f"📖 Теория сгенерирована: {len(theory)} символов")
        return theory
        
    except Exception as e:
        logger.error(f"❌ Ошибка генерации теории для теста: {e}")
        return f"Тема '{topic}' рассматривается в руководствах по цифровой грамотности."
...
def generate_contextual_test(topic, relevant_sections):
    """Генерация теста с контекстным анализом - ГАРАНТИРОВАННО 5 ВОПРОСОВ"""
    ...
    try:
        ...
        # Генерируем ГАРАНТИРОВАННО 5 вопросов
        questions = generate_contextual_questions(topic, relevant_sections, theory)
        ...
        return {
            'topic': topic,
            'theory': theory,
            'questions': questions[:5],  # Гарантируем максимум 5 вопросов
            'sources': {
